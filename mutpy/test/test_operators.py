...
class OperatorTestCase(unittest.TestCase):
    def assert_mutation(
        ...
        for mutation, mutant in operator.mutate(
            original_ast, coverage_injector=coverage_injector, module=module
        ):
    ...
    def assert_location(self, mutant):
        for node in ast.walk(mutant):
            if "lineno" in node._attributes:
                self.assertTrue(
                    hasattr(node, "lineno"), "Missing lineno in " + str(node)
                )
            if "col_offset" in node._attributes:
                self.assertTrue(
                    hasattr(node, "col_offset"), "Missing col_offset in " + str(node)
                )
...
class ConstantReplacementTest(OperatorTestCase):
    ...
    def test_not_mutate_function(self):
        self.assert_mutation(
            "@notmutate" + EOL + "def x():" + EOL + INDENT + "'ham'", []
        )

    def test_not_mutate_class(self):
        self.assert_mutation(
            "@notmutate" + EOL + "class X:" + EOL + INDENT + "'ham'", []
        )
...
class MutateOnlyCoveredNodesTest(OperatorTestCase):
    ...
    def test_not_covered_if_node(self):
        self.assert_mutation(
            "if False:" + EOL + INDENT + "if False:" + EOL + 2 * INDENT + PASS,
    ...
    def test_not_covered_while_node(self):
        self.assert_mutation(
            "while False:" + EOL + INDENT + "while False:" + EOL + 2 * INDENT + PASS,
            [
                "while not False:"
                + EOL
                + INDENT
...
class StatementDeletionTest(OperatorTestCase):
    ...
    def test_return_deletion(self):
        self.assert_mutation(
            "def f():" + EOL + INDENT + "return 1", ["def f():" + EOL + INDENT + PASS]
        )
...
class ZeroIterationLoopTest(OperatorTestCase):
    ...
    def test_while_zero_iteration(self):
        self.assert_mutation(
            "while x:" + EOL + INDENT + PASS, ["while x:" + EOL + INDENT + "break"]
        )
