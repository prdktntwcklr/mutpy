...
def build_parser():
    ...
    parser.add_argument("--timeout-factor", "-f", type=float, default=5,
                        help="max timeout factor (default: %(default)s)")
...
def build_mutator(cfg):
    ...
    if cfg.operator:
        operators_set |= {get_operator(name, name_to_operator) for name in cfg.operator}
    else:
