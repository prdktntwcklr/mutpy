...
From the [relevant article on Wikipedia](https://en.wikipedia.org/wiki/Mutation_testing):
...
MutPy output. In this case MutPy didn\'t generate any incompetent
(raised `TypeError`) and timeout (generated infinite loop) mutants. Our
...
-   `--runner RUNNER` - currently supported are: unittest (default),
    pytest (experimental)
